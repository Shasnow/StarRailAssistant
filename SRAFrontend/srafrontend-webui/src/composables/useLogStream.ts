...
// ---------- 模块级单例状态 ----------
// 状态不随组件卸载销毁：离开日志页后 SSE 连接与日志缓冲保持存活，
// 切换路由期间后端推送的日志持续收集，切回时无缝衔接（否则日志会丢失）。
...
/** 模块加载时从 localStorage 恢复最近日志（缓存损坏时静默忽略，从空白开始） */
...
/**
 * 确保连接可用：已打开或正在建连/重连中则不动（避免切回页面时
 * 强制断开重连丢消息），仅在真正未连接（如重连耗尽停在 disconnected）时建连
 */
