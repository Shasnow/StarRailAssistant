...
public class CommonModel(SettingsService settingsService, UpdateService updateService,
    PythonService pythonService, ILogger<CommonModel> logger, ISukiToastManager toastManager)
{
    public async Task CheckForUpdatesAsync()
    {
        ...
        var response = await updateService.CheckForUpdatesAsync(currentVersion.ToString(), cdk, channel);
        if (response == null)
        {
    ...
    private async Task DownloadUpdateAsync(VersionResponse versionResponse)
    {
        ...
        try
        {
            result = await updateService.DownloadUpdateAsync(versionResponse, downloadChannel, progressHandler,
                proxies);
    ...
    private async Task<bool> FixPythonWithRetryAsync(int maxRetries = 2)
    {
        for (var retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                ...
                var (tempZipPath, errorMessage) = await pythonService.DownloadPythonAsync(pythonZipUrl, md5Sum, progressHandler);
                toastManager.Dismiss(toast);
    ...
    private async Task<bool> FixRequirementsWithRetryAsync(int maxRetries = 2)
    {
        for (var retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                logger.LogInformation("Starting requirements fix (Attempt {Retry}/{MaxRetries})", retry + 1, maxRetries);
