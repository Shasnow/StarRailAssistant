...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    public MainWindowViewModel(IEnumerable<PageViewModel> pages, ISukiToastManager toastManager,
        ...
        PythonService pythonService, ILogger<MainWindowViewModel> logger)
    {
        ...
        _ = CheckAndFixPythonEnvironmentAsync();
    }
    ...
    private async Task CheckForUpdatesAsync()
    {
        ...
        if (response == null)
        {
            _logger.LogError("Failed to check for updates: response is null");
        ...
        {
            ShowSuccessToast("已是最新版本", $"当前版本：{Settings.Version}");
            return;
        }
    ...
    private async Task DownloadUpdateAsync(VersionResponse versionResponse)
    {
        ...
        var toast = CreateStandardToastBuilder("正在下载...", progressPanel, NotificationType.Information).Queue();
        ...
        ShowSuccessToast("下载完成", $"更新包将在3秒后解压到当前目录");
        await Task.Delay(3000);
        ZipUtil.UnzipExternal(result, Environment.CurrentDirectory);
    ...
    /// </summary>
    private async Task CheckAndFixPythonEnvironmentAsync()
    {
        try
        {
            // 移除不必要的延迟：应用启动后无需等待1秒，直接检查环境
            await Task.Delay(500);
            _logger.LogInformation("Starting Python environment check...");
            var pythonEnvStatus = _pythonService.CheckPythonEnvironment();
            _logger.LogInformation("Python EnvStatus: {Status}", pythonEnvStatus);

            // 环境正常，直接返回
            if (pythonEnvStatus == PythonEnvStatus.AllSet)
            {
                _logger.LogInformation("Python environment is all set.");
            ...
            if (!userAgreed)
            {
                _logger.LogInformation("User canceled Python environment fix.");
        ...
        {
            _logger.LogError(ex, "Python Environment check and fix failed unexpectedly");
    ...
    private async Task<bool> ExecuteFixFlowAsync(PythonEnvStatus status)
    {
        try
        {
            ...
            switch (status)
            {
                ...
                default:
                    _logger.LogWarning("Unknown Status：{Status}", status);
                    return false;
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "ExecuteFixFlowAsync failed unexpectedly");
            return false;
        }
    }
    ...
    private async Task<bool> FixPythonWithRetryAsync(int maxRetries = 2)
    {
        for (var retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                _logger.LogInformation("Starting Python fix (Attempt {Retry}/{MaxRetries})", retry + 1, maxRetries);
                
                // 1. 构建进度 UI
                var (progressPanel, progressBar, sizeLabel) = BuildDownloadProgressUi();
                var toastBuilder = CreateStandardToastBuilder(PythonDownloadTitle, progressPanel, NotificationType.Information);
                var toast = toastBuilder.Queue();

                // 2. 下载 Python
                var md5Sum = _settingsService.Settings.PythonMd5;
                var pythonZipUrl = _settingsService.Settings.PythonDownloadPath;
                var progressHandler = new Progress<DownloadStatus>(value =>
                {
                    progressBar.Value = value.ProgressPercent;
                    sizeLabel.Content = $"{value.FormattedDownloadedSize} / {value.FormattedTotalSize} | {value.FormattedSpeed}";
                ...
                var (tempZipPath, errorMessage) = await _pythonService.DownloadPythonAsync(pythonZipUrl, md5Sum, progressHandler);
                ToastManager.Dismiss(toast);

                // 3. 检查下载结果
                if (string.IsNullOrEmpty(tempZipPath))
                {
                    _logger.LogError("Python Download Error：{ErrorMessage}", errorMessage);
                ...
                _pythonService.EnableSiteModule();
                _logger.LogInformation("Python Fix Successful");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Python Fix Attempt {Retry} Failed", retry + 1);
                if (retry < maxRetries - 1)
                {
    ...
    private async Task<bool> FixPipWithRetryAsync(int maxRetries = 2)
    {
        for (var retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                _logger.LogInformation("Starting pip fix (Attempt {Retry}/{MaxRetries})", retry + 1, maxRetries);
                
                // 1. 构建进度 UI
                var (progressPanel, progressBar, sizeLabel) = BuildDownloadProgressUi();
                var toastBuilder = CreateStandardToastBuilder(PipInstallTitle, progressPanel, NotificationType.Information);
                var toast = toastBuilder.Queue();
                ...
                var installSuccess = await _pythonService.InstallPipAsync(progressHandler);
                ToastManager.Dismiss(toast);

                if (!installSuccess)
                {
                    _logger.LogError("pip Installation Failed (Attempt {Retry})", retry + 1);
                    if (retry < maxRetries - 1)
                    {
                ...
                await _pythonService.SetupPipAsync();
                
                // 二次校验 pip 是否安装成功
                if (!_pythonService.CheckPipInstalled())
                {
                    _logger.LogError("pip Configuration Failed After Installation");
                    if (retry < maxRetries - 1)
                    {
                ...
                _logger.LogInformation("pip Fix Successful");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "pip Fix Attempt {Retry} Failed", retry + 1);
                if (retry < maxRetries - 1)
                {
    ...
    private async Task<bool> FixRequirementsWithRetryAsync(int maxRetries = 2)
    {
        for (var retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                _logger.LogInformation("Starting requirements fix (Attempt {Retry}/{MaxRetries})", retry + 1, maxRetries);
                
                // 1. 构建进度 UI
                var logLabel = new Label { Content = "正在连接 PyPI 源..." };
                var toastBuilder = CreateStandardToastBuilder(RequirementsInstallTitle, logLabel, NotificationType.Information);
                toastBuilder.SetDismissAfter(TimeSpan.FromMinutes(10)); // 延长超时，避免安装时间过长被自动关闭
                var toast = toastBuilder.Queue();

                // 2. 安装依赖
                var progressHandler = new Progress<string>(log =>
                {
                    logLabel.Content = log;
                });

                var installSuccess = await _pythonService.InstallRequirementsAsync(progressHandler);
                ToastManager.Dismiss(toast);

                if (!installSuccess)
                {
                    _logger.LogError("Requirements Installation Failed (Attempt {Retry})", retry + 1);
                    if (retry < maxRetries - 1)
                    {
                ...
                _logger.LogInformation("Requirements Fix Successful");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Requirements Fix Attempt {Retry} Failed", retry + 1);
                if (retry < maxRetries - 1)
                {
    ...
    private async Task<bool> UnzipPythonAsync(string zipPath)
    {
        try
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    Directory.Delete(PathString.PythonDir, recursive: true);
                }
            ...
            _logger.LogError("Python Unzip Failed: python.exe not found after extraction");
            ShowErrorToast("解压失败", "压缩包解压不完整，未找到 Python 可执行文件");
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Python Unzip Failed with Exception");
    ...
    private void CleanupPythonTempFiles()
    {
        try
        {
            ...
            _logger.LogInformation("Successfully cleaned up Python temporary files.");
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to clean up Python temporary files.");
        }
    }
    #endregion
}