...
class Handler(ABC):
    ...
    def format_message(self, level, message):
        return self.fmt.format(
            ...
            module=sys._getframe(2).f_code.co_filename  # 调用日志方法的模块
        )
...
class ConsoleHandler(Handler):
    ...
    def __init__(self, fmt: str = "{time} {thread} {level} {message}", level: Level = Level.TRACE) -> None:
    ...
    def format_message(self, level, message):
        return self.fmt.format(
            time=time.strftime('%Y-%m-%d %H:%M:%S'),
            level=f"[{level.lower()}]{level}[/{level.lower()}]",
            message=message,
            thread=threading.current_thread().name,
            module=sys._getframe(2).f_code.co_filename  # 调用日志方法的模块
        )
...
class FileHandler(Handler):
    ...
    def __init__(self, file_path: str, fmt: str = "{time} {thread} {level} {message}",
                 level: Level = Level.TRACE) -> None:
        super().__init__(fmt, level)
