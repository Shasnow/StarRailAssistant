...
class AutoPlotTrigger(BaseTrigger):
    ...
    def run(self):
        ...
        for i in range(5, 0, -1):
            if self.operator.locate(f"resources/img/{i}.png", from_x=0.63, from_y=0.375, to_x=0.67, to_y=0.72, trace=False):
    ...
    def skip_check(self):
        ...
        if self.operator.click_img('resources/img/ensure.png'):  # 如果出现确认跳过按钮，说明可以跳过
            self.can_skip = True
        else:
            self.can_skip = False
